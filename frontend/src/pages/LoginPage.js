...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErrors({});
      setLoading(false);
      navigate("/homepage");  // Navigate to home on login
    } catch (err) {
      setLoading(false);
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-start bg-purple-500 relative overflow-hidden py-12">
      ...
      <div className="bg-white rounded-lg shadow-xl p-8 w-full max-w-md relative z-10">
        ...
        <p className="text-center text-gray-600 mb-6">Access your LunaQ account</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <button
              ...
              disabled={loading}
            >
              {loading ? <FaSpinner className="animate-spin h-5 w-5 mr-3" /> : "Log in"}
            </button>
          </div>
        </form>
